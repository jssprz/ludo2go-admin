...
import { applyVariantPriceRule } from '@/lib/pricing/variant-price-rules';
...
export async function POST(_request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const result = await applyVariantPriceRule(id, adminUserId);
    ...
    return NextResponse.json(
      { error: error?.message || 'Failed to apply price rule' },
      { status: 500 }
    );
  }
}
