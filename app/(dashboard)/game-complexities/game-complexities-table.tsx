...
type Props = {
  initialComplexities: GameComplexity[];
};

export function GameComplexitiesTable({ initialComplexities }: Props) {
  const router = useRouter();
  const t = useTranslations('gameComplexities');
  const tc = useTranslations('common');
  const [complexities, setComplexities] = useState<GameComplexity[]>(initialComplexities);
  ...
  const [selectedComplexity, setSelectedComplexity] = useState<GameComplexity | null>(null);
  ...
  const filteredComplexities = complexities.filter(
    (mechanic) =>
  ...
  function handleNameChange(name: string) {
    setFormName(name);
    if (!selectedComplexity || !formSlug) {
  ...
  function resetForm() {
    ...
    setFormDescription('');
    setFormIcon('');
    setFormOrder(complexities.length);
  ...
  function openEditDialog(mechanic: GameComplexity) {
    setSelectedComplexity(mechanic);
  ...
  function openDeleteDialog(mechanic: GameComplexity) {
    setSelectedComplexity(mechanic);
  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch('/api/game-complexities', {
        method: 'POST',
  ...
  async function handleUpdate() {
    ...
    try {
      const res = await fetch(`/api/game-complexities/${selectedComplexity.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: formName.trim(),
      ...
      setComplexities((prev) =>
        prev
          .map((m) =>
            m.id === selectedComplexity.id ? { ...data, _count: m._count } : m
          )
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setComplexities((prev) => prev.filter((m) => m.id !== selectedComplexity.id));
  ...
  async function handleToggleActive(mechanic: GameComplexity) {
    setIsLoading(true);

    try {
      const res = await fetch(`/api/game-complexities/${mechanic.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: mechanic.name,
    ...
    <>
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle>{t('allComplexities')}</CardTitle>
              <CardDescription>
                {complexities.length} {t('complexitiesTotal', { count: complexities.length })}
              </CardDescription>
            </div>
                ...
                {filteredComplexities.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                      {search ? t('noResultsSearch') : t('noComplexities')}
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredComplexities.map((mechanic) => (
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{t('addComplexity')}</DialogTitle>
            <DialogDescription>
              {t('createDescription')}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="create-active">{t('activeComplexity')}</Label>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{t('editComplexity')}</DialogTitle>
            <DialogDescription>
              {t('editDescription')}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="rounded border-gray-300"
              />
              <Label htmlFor="edit-active">{t('activeComplexity')}</Label>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('deleteComplexity')}</DialogTitle>
            <DialogDescription>
              {t('confirmDelete', { name: selectedComplexity?.name || '' })}
            </DialogDescription>
          </DialogHeader>
          {formError && (
            <div className="rounded-md bg-red-50 p-3 text-sm text-red-600">
          ...
          {selectedComplexity && selectedComplexity._count.games > 0 && (
            <div className="rounded-md bg-yellow-50 p-3 text-sm text-yellow-800">
              {t('cannotDeleteWithGames', { count: selectedComplexity._count.games })}
            </div>
          )}
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isLoading || (selectedComplexity?._count.games ?? 0) > 0}
            >
              {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {tc('delete')}
            </Button>
