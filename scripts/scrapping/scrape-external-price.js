// scrape-external-price.js
// Run: node scrape-external-price.js <variantId> <productUrl>
//      (first: npm i playwright @prisma/client && npx playwright install)

const { chromium, selectors } = require('playwright');
const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

// Templates / helpers (same as before)
const TEMPLATES = {
...
function findOffer(node) {
  ...
  if (node.offers) {
    if (Array.isArray(node.offers)) {
      return node.offers.find(o => o.price) || node.offers[0];
    }
    return node.offers;
  }
...
function normalizePriceText(text, locale = 'es-CL') {
...
async function extractWithTemplates(page, hostname) {
  const candidates = [...(TEMPLATES[hostname] || []), ...TEMPLATES.__woocommerce_defaults__];
...
// CLI: node scrape-external-price.js <variantId> <productUrl>
(async () => {
  const variantId = process.argv[2];
  const url = process.argv[3];
  if (!variantId || !url) {
    console.error('Usage: node scrape-external-price.js <variantId> <productUrl>');
    process.exit(1);
  }
  ...
  try {
    ...
    if (result.price == null) {
      console.error('Price not found, aborting DB insert.');
      process.exit(2);
    }

    const parsed = new URL(url);
    const hostname = parsed.hostname.toLowerCase();
    // Try to find a store by matching hostname to store.url
    const store = await prisma.store.findFirst({ where: { url: { contains: hostname } } });
    if (!store) {
      console.error(`Store not found for hostname ${hostname}. Create a Store entry first.`);
      process.exit(3);
    }

    // Build urlPathInStore from pathname + search
    const urlPathInStore = parsed.pathname + (parsed.search || '');

    // Insert observed price
    await prisma.itemPriceInStore.create({
      data: {
        variantId: variantId,
        storeId: store.id,
        urlPathInStore: urlPathInStore,
